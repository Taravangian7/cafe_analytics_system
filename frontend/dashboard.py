...
def dashboard_page():
    ...
    st.title("☕ Dashboard")
    st.caption("Base de datos del negocio")
    st.code(user["db_name"])
    ...
    if not is_logged_in():
        ...
        with tab2:
            if datos_ini:
                ...
                st.error(message_datos_iniciales)
        ...
        with tab4:
            if datos_ini:    
                ...
                carga_plato_y_receta_manual(conn,prefix='cargaplatos2')
            else:
                st.error(message_datos_iniciales)
        with tab5:
            if datos_ini:
